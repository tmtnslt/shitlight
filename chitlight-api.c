...
void add_frame(rep, t_chitframe* frame); // align a frame to optimal memory layout and add to the ring buffer
                                         // to be drawn (rep) times. This call will block if the ring buffer is full

int try_add_frame(rep t_chitrame* frame); // same as add_frame, but don't block. Returns 1 on success and 0 if buffer is full
